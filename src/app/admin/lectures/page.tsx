...
// Lectures THIS admin scheduled themselves (routes/lectures.js's GET /mine —
// same endpoint course reps and teachers use for their own). An admin can
// also cancel a lecture ANYONE scheduled (see the backend route), which this
// page doesn't surface — it's a school-wide oversight capability, not
// something that needs its own "every lecture in the system" browser to be
// useful; the per-subject dashboard (admin/reports) already covers that.
export default async function AdminLecturesPage() {
  ...
  return (
    <div>
      <PageHeader
        title="Your lectures"
        subtitle="Lectures you've scheduled yourself, for any subject."
        action={
          <Link href="/admin/lectures/new" className={buttonClass}>
            + Schedule lecture
          </Link>
        }
      />
